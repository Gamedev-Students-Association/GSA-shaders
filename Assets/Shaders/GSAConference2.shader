Shader "Custom/GSAConference2"
{
    ...
    SubShader
    {
        ...
        uniform float4 _GridTranslation; //float2
        uniform float _GridRotation;
        uniform float4 _GridTiltX;
        uniform float4 _GridTiltY;
        uniform float4 _GridScale; //float2
        uniform float4 _CubeSize; //float2
        uniform float _CubeCornerSize;
        uniform float _HueShift;
        uniform float _HueStatic;
        uniform float4 _HueSign;
        uniform float4 _HueScale; //actually vect4, every one another direction
        uniform float4 _HueExponenta;
        uniform float _CenterCubesValue;
        uniform float4 _CubesSaturation; //float2 (range)
        uniform float4 _CubesValue; //float2 (range)
        ...
        uniform float4 _LineASpread;
        uniform float4 _LineBSpread;

        uniform float4 _CompoundLineRange;
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            //center transform
            float2 transform = IN.uv_MainTex - float2(0.5, 0.5);
